...
# Source: https://github.com/facebookresearch/SlowFast/blob/main/slowfast/datasets/mixup.py
# License: Apache License 2.0 (see https://github.com/facebookresearch/SlowFast/blob/main/LICENSE)

import numpy as np


def mixup_target_binary(target, lam=1.0, smoothing=0.0):
    """
    Apply mixup to binary targets (0 or 1).
    Args:
        target (torch.Tensor): Class labels (0 or 1).
        lam (float): lambda value for mixup/cutmix.
        smoothing (float): Label smoothing value.
    """
    off_value = smoothing / 2.0
    on_value = 1.0 - smoothing + off_value
    target1 = target.float() * on_value + (1.0 - target.float()) * off_value
    target2 = target.flip(0).float() * on_value + \
        (1.0 - target.flip(0).float()) * off_value
...
def get_cutmix_bbox(img_shape, lam, correct_lam=True, count=None):
    """
    Generate the box coordinates for cutmix.
...
class MixUpBinary:
    """
    Apply mixup and/or cutmix for volumes at batch level for binary classification.
    ...
    def __call__(self, x, target):
        ...
        target = mixup_target_binary(target, lam, self.label_smoothing)
        return x, target
